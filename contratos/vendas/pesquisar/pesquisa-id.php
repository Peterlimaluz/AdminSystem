...
<html>
<head>
<title>Reparsul | Pesquisa :: ID</title>
...
if (!$begin) { $begin = 0; }
$id = $_POST['reg'];						
$query = "SELECT COUNT(*) FROM contrato_vendas WHERE ve_id LIKE '%$id%'";
$query = mysql_query($query);
$query = mysql_fetch_array($query);
$total = $query[0];
...
$id = $_POST['reg'];
$query = "SELECT DISTINCT * FROM contrato_vendas WHERE ve_id LIKE '%$id%'";
$query = mysql_query($query);
...
?> 
							<tbody> 
								<tr> 
									<th scope="row" style="background-color: aquamarine"><?php echo $linha['ve_id'] ?></th> 
