...
echo"
		...
		<td>Modo de Pagamento:(<b>$tipo</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$tipo= $_POST['pag'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_pag LIKE '%$tipo%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_pag LIKE '%$tipo%'");
