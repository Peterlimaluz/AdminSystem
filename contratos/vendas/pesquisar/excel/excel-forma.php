...
echo"
		...
		<td>Forma de Pagamento:(<b>$forma</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$forma = $_POST['forma'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_forma LIKE '%$forma%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_forma LIKE '%$forma%'");
