...
echo"
		...
		<td>Data de Venda:(<b>$inicio</b>)-(<b>$fim<b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$inicio = $_POST['dtinicio'];
		$fim = $_POST['dtfim'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_dtvenda BETWEEN ('$inicio') AND ('$fim')");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_dtvenda BETWEEN ('$inicio') AND ('$fim')");
