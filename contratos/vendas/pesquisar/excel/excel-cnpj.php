...
echo"
		...
		$cpf = $_POST['cpf'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas contrato_vendas WHERE ve_cpf LIKE '%$cpf%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas contrato_vendas WHERE ve_cpf LIKE '%$cpf%'");
