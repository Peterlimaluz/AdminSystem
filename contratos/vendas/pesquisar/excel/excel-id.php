...
echo"
		...
		<td>Código do Contrato:(<b>$id</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$id = $_POST['reg'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_id LIKE '%$id%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_id LIKE '%$id%'");
