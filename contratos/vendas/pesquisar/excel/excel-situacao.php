...
$query = mysql_query($query);
   ...
   $dataTwo = "$day[2]/$day[1]/$day[0]";
       ...
       echo"
		...
		$situacao = $_POST['situacao'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_situacao LIKE '%$situacao%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_situacao LIKE '%$situacao%'");
