...
echo"
		...
		$inicio = $_POST['dtinicio'];
$fim = $_POST['dtfim'];
		$query = mysql_query("SELECT SUM(REPLACE(ve_total,'.','')) as soma FROM contrato_vendas WHERE ve_dtentrega BETWEEN ('$inicio') AND ('$fim')");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas WHERE ve_dtentrega BETWEEN ('$inicio') AND ('$fim')");
