...
echo"
   ...
   $query = 'SELECT DISTINCT * vendas ASC';
   $query = mysql_query($query);

    //varremos o array com o foreach para pegar os dados
    while ($linha = mysql_fetch_assoc($query)) {

   $var = $linha['cove_dtvenda'];
   $var = explode(" ",$var);
   $dia = $var[0];
   $dia = explode("-",$dia);
   $data = "$dia[2]/$dia[1]/$dia[0]";
		
   $twodt = $linha['cove_dtentrega'];
   $twodt = explode(" ",$twodt);
   $day = $twodt[0];
   $day = explode("-",$day);
   $dataTwo = "$day[2]/$day[1]/$day[0]";
		...
		$query = mysql_query("SELECT SUM(REPLACE(cove_total,'.','')) as soma FROM contrato_vendas");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(cove_desVt,'.','')) as desconto FROM contrato_vendas");
