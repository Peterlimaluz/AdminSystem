...
$fim = $_POST['dtFinal'];
$query = "SELECT DISTINCT * FROM manutencao WHERE ma_dtserv BETWEEN ('$inicio') AND ('$fim') ORDER BY ma_id DESC";
  ...
  $var = $linha['ma_dtgerado'];
   ...
   <td>".$linha['ma_id']."</td>
    ...
    <td>".$linha['ma_cidade']."</td>
		...
		<td>Data do Serviço:(<b>$inicio</b>)-(<b>$fim</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$inicio = $_POST['dtInicio'];
		$fim = $_POST['dtFinal'];
		$query = mysql_query("SELECT SUM(REPLACE(ma_vTotal,'.','')) as soma FROM manutencao WHERE ma_dtserv BETWEEN ('$inicio') AND ('$fim') ORDER BY ma_id DESC");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(ma_vDesconto,'.','')) as desconto FROM manutencao WHERE ma_dtserv BETWEEN ('$inicio') AND ('$fim') ORDER BY ma_id DESC");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(ma_vAcrescimo,'.','')) as acrescimo FROM manutencao WHERE ma_dtserv BETWEEN ('$inicio') AND ('$fim') ORDER BY ma_id DESC");
