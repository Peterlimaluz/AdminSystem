...
//incluimos nossa conexão
    ...
    //mandamos nossa query para nosso método dentro de conexao dando um return $stmt->fetchAll(PDO::FETCH_ASSOC);
   $cpf = $_POST['cpf'];
$query = "SELECT DISTINCT * FROM manutencao WHERE ma_cpf LIKE '%$cpf%' ORDER BY ma_id DESC";
  ...
  $var = $linha['ma_dtgerado'];
   ...
   <td>".$linha['ma_id']."</td>
	...
	echo"
		...
		$cpf = $_POST['cpf'];
		$query = mysql_query("SELECT SUM(REPLACE(ma_vTotal,'.','')) as soma FROM manutencao WHERE ma_cpf LIKE '%$cpf%' ORDER BY ma_id DESC");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(ma_vDesconto,'.','')) as desconto FROM manutencao WHERE ma_cpf LIKE '%$cpf%' ORDER BY ma_id DESC");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(ma_vAcrescimo,'.','')) as acrescimo FROM manutencao WHERE ma_cpf LIKE '%$cpf%' ORDER BY ma_id DESC");
