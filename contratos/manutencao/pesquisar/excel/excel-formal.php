...
//incluimos nossa conexão
    ...
    //mandamos nossa query para nosso método dentro de conexao dando um return $stmt->fetchAll(PDO::FETCH_ASSOC);
   $forma = $_POST['faforma'];
$query = "SELECT DISTINCT * FROM manutencao WHERE ma_forma LIKE '%$forma%' ORDER BY ma_id DESC";
  ...
  $var = $linha['ma_dtgerado'];
   ...
   <td>".$linha['ma_id']."</td>
	...
	echo"
		...
		<td>Forma de Contrato:(<b>$forma</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$forma = $_POST['faforma'];
		$query = mysql_query("SELECT SUM(REPLACE(ma_vTotal,'.','')) as soma FROM manutencao WHERE ma_forma LIKE '%$forma%' ORDER BY ma_id DESC");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(ma_vDesconto,'.','')) as desconto FROM manutencao WHERE ma_forma LIKE '%$forma%' ORDER BY ma_id DESC");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(ma_vAcrescimo,'.','')) as acrescimo FROM manutencao WHERE ma_forma LIKE '%$forma%' ORDER BY ma_id DESC");
