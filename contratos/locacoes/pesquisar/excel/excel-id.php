...
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_id LIKE '%$contrato%'";
   ...
   echo "
    ...
    echo"
		...
		<tr>
			...
			<td>Código do Contrato:(<b>$contrato</b>)</td>
			</tr>
			<tr>
			</tr>
			<tr>;
		...
		$contrato = $_POST['contrato'];
		$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_id LIKE '%$contrato%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_id LIKE '%$contrato%'");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_id LIKE '%$contrato%'");
