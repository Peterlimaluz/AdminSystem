...
//incluimos nossa conexão
    ...
    //mandamos nossa query para nosso método dentro de conexao dando um return $stmt->fetchAll(PDO::FETCH_ASSOC);
   $forma = $_POST['faforma'];
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_faforma LIKE '%$forma%'";
   ...
   echo "
    ...
    echo"
		...
		<tr>
			...
			<td>Forma de Pagamento:(<b>$faforma</b>)</td>
			</tr>
			<tr>
			</tr>
			<tr>
		...
		$forma = $_POST['faforma'];
		$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_faforma LIKE '%$forma%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_faforma LIKE '%$forma%'");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_faforma LIKE '%$forma%'");
