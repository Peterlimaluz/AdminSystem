...
//incluimos nossa conexão
    ...
    //mandamos nossa query para nosso método dentro de conexao dando um return $stmt->fetchAll(PDO::FETCH_ASSOC);
   $tipo = $_POST['tipo'];
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_tipo LIKE '%$tipo%'";
   ...
   $dataR = "$diaR[2]/$diaR[1]/$diaR[0]";
	...
	echo"
		...
		<td>Dados Utilizados para Gerar o Relatório:</td>
			<td>Tipo de Aluguel:(<b>$tipo</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$tipo = $_POST['tipo'];
		$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_tipo LIKE '%$tipo%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_tipo LIKE '%$tipo%'");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_tipo LIKE '%$tipo%'");
