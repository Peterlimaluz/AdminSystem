...
$fim = $_POST['dtFinal'];
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_rdentrega BETWEEN ('$inicio') AND ('$fim')";
   ...
   $dataR = "$diaR[2]/$diaR[1]/$diaR[0]";
	...
	echo"
		...
		<tr>
		 ...
		 <td>Data de Entrega:(<b>$inicio</b>)-(<b>$fim</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$inicio = $_POST['dtInicio'];
		$fim = $_POST['dtFinal'];
		$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_rdentrega BETWEEN ('$inicio') AND ('$fim')");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_rdentrega BETWEEN ('$inicio') AND ('$fim')");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_rdentrega BETWEEN ('$inicio') AND ('$fim')");
