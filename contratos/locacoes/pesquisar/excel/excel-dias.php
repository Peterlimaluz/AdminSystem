...
while ($linha = mysql_fetch_array($query)) {
  ...
  $var = $linha['lo_dtabertura'];
   ...
   echo "
	...
	<td>Dia de Pagamento:(<b>$dia</b>)</td>
	</tr>
	<tr>
	</tr>
	<tr>
	...
	$dia = $_POST['fadia'];
	$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_fadia LIKE '%$dia%'");

	$cont = mysql_fetch_array($query);

	$total = $cont["soma"];

	$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_fadia LIKE '%$dia%'");
	$cont2 = mysql_fetch_array($query2);
	$descont = $cont2["desconto"];

	$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_fadia LIKE '%$dia%'");
