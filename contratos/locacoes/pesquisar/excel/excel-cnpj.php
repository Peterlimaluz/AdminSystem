...
//incluimos nossa conexão
    ...
    //mandamos nossa query para nosso método dentro de conexao dando um return $stmt->fetchAll(PDO::FETCH_ASSOC);
   $cpf = $_POST['cpf'];
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_cpf LIKE '%$cpf%'";
...
while ($linha = mysql_fetch_array($query)) {
  ...
  $var = $linha['lo_dtabertura'];
   ...
   $dataR = "$diaR[2]/$diaR[1]/$diaR[0]";
	...
	$cpf = $_POST['cpf'];
	$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_cpf LIKE '%$cpf%'");

	$cont = mysql_fetch_array($query);

	$total = $cont["soma"];

	$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_cpf LIKE '%$cpf%'");
	$cont2 = mysql_fetch_array($query2);
	$descont = $cont2["desconto"];

	$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_cpf LIKE '%$cpf%'");
