...
$query = "SELECT DISTINCT * FROM contrato_locacao WHERE lo_situacao LIKE '%$situacao%'";
  ...
  $var = $linha['lo_dtabertura'];
   ...
   echo "
	...
	<td>Dados Utilizados para Gerar o Relatório:</td>
		<td>Situação do Contrato:(<b>$situacao</b>)</td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		$situacao = $_POST['situacao'];
		$query = mysql_query("SELECT SUM(REPLACE(lo_total,'.','')) as soma FROM contrato_locacao WHERE lo_situacao LIKE '%$situacao%'");

		$cont = mysql_fetch_array($query);

		$total = $cont["soma"];

		$query2 = mysql_query("SELECT SUM(REPLACE(lo_desconto,'.','')) as desconto FROM contrato_locacao WHERE lo_situacao LIKE '%$situacao%'");
		$cont2 = mysql_fetch_array($query2);
		$descont = $cont2["desconto"];

		$query3 = mysql_query("SELECT SUM(REPLACE(lo_acrescimo,'.','')) as acrescimo FROM contrato_locacao WHERE lo_situacao LIKE '%$situacao%'");
