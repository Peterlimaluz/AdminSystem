...
$fim = $_POST['dtFinal'];
$query = "SELECT DISTINCT * FROM orcamento WHERE orca_dtgerado BETWEEN ('$inicio') AND ('$fim') ORDER BY orca_id ASC";
...
echo"
		...
		$inicio = $_POST['dtInicio'];
		$fim = $_POST['dtFinal'];
		$query = mysql_query("SELECT SUM(REPLACE(orca_valTotal,'.','')) as soma FROM orcamento WHERE orca_dtgerado BETWEEN ('$inicio') AND ('$fim') ORDER BY orca_id ASC");
